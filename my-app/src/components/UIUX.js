...
const HomePage = () => {
 ...
 const [isSidebarActive, setIsSidebarActive] = useState(false);
  ...
  // Fetch cards of category 'App Design' on page load
 // Frontend useEffect with proper category filtering
useEffect(() => {
    const fetchCards = async () => {
      try {
        // Correct the template string syntax and ensure category is passed properly
        const category = "App Design";  // Example of hardcoded category, or you can make it dynamic
        const response = await axios.get(`http://localhost:5000/api/categorycards?category=${encodeURIComponent(category)}`);
    ...
    <>
      ...
      <nav className="navbar">
        ...
        <button
          ...
          onClick={() => navigate("/account")} // Navigate to /account on click
        >
          Account
        </button>
      </nav>
